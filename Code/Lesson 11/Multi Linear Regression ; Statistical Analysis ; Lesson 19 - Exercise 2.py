...
df = pd.read_csv(os.path.dirname(os.path.realpath(__file__))+"\\Datasets\\Heart.data.csv")


variables = ['biking', 'smoking']
for var in variables:
    plt.figure() 
    sns.regplot(x=var, y='heart.disease', data=df).set(title=f'Regression plot of {var} and heart desease test result');


#calculate the correlation of the new variables, this time using Seaborn's heatmap()    
correlations = df.corr()
# annot=True displays the correlation values
# sns.heatmap(correlations, annot=True).set(title='Heatmap of Test results biking & smoking Data - Pearson Correlations');
...
print(f'Root mean squared error: {rmse:.2f}') #the lower the Root mean squared error the better the prediction
